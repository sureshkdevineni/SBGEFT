"""
Data loaders module
Contains functions to load BAO, Pantheon+, and DES-SN5YR observational data
"""
...
def load_bao_gaussian(mean_path, cov_path, zmin, zmax, include_lyalpha=False):
    ...
    if not np.any(mask):
        raise ValueError("BAO z-window/Ly-alpha mask removed all entries")
...
def load_pantheon_plus(data_file, cov_file, zmin, zmax):
    ...
    # Official low-z cut + user z-range
    mask = (
        (df["zHD"] > 0.01) &
        (df["zHD"] >= zmin) &
        (df["zHD"] <= zmax)
    ...
    # Use np.asarray to avoid static analyzer issues with pandas.Series.to_numpy
...
def load_des_sn_hd_and_cov(
    ...
    # Use np.asarray / dtype cast to avoid pandas .to_numpy static-analysis issues
